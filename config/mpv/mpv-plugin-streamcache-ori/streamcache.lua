...
function streamcache_check_fill()
    ...
    if ( current_speed < streamcache_min_speed ) or ( current_speed > math.floor((10000.0/streamcache_min_speed) + 0.5)/10000.0 ) then
        mp.msg.log("debug", "Manual change detected, speed outside of automatic range")
        return
    end
    ...
    if (cache_used <= 0 or (cache_used < (streamcache_cache_high-streamcache_cache_low) and current_speed > 1.0)) then
        ...
        if (new_speed ~= current_speed) then
            mp.msg.log("debug", "cache-used " .. cache_used .. " kB, lowered speed to " .. new_speed)
    ...
    if (cache_used > streamcache_cache_high or ((cache_used > streamcache_cache_low) and current_speed < 1.0)) then
        ...
        if (new_speed ~= current_speed) then
            mp.msg.log("debug", "cache-used " .. cache_used .. " kB, increased speed to " .. new_speed)
            mp.set_property("speed", new_speed)
        end
        return
    end
...
function streamcache_on_loaded()
    mp.msg.log("v", "new file loaded - starting with minimum speed = " .. streamcache_min_speed)
