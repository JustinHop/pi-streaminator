...
local function add_single_video(json)
    ...
    msg.info("streamurl: " .. streamurl)

    local meta = {}
    -- msg.info(utils.to_string(meta))

    if not (json.description == nil) then
        local desc = string.gsub(json.description, "\n\n", "\n")
        msg.info("description:" .. desc)
        -- mp.set_property("metadata/Description", desc)
        -- mp.set_property("file-local-options/description", desc)
        meta.description = desc
        for suburl in desc:gmatch("https://newtube.app/[^%s]+/[^%s]+") do
            pcall(function ()
                msg.info("BrandNewTubeLink: ", suburl)
                local appendstr = "append"
                local pos = mp.get_property_number('playlist-pos', 0)
                local plen = mp.get_property_number('playlist-count', 0)
                mp.commandv("loadfile", suburl, appendstr)
                mp.commandv("playlist-move", plen,  pos+1)
            end)
        end
        for suburl in desc:gmatch("https://rumble.com/[^%s]+.html") do
            pcall(function ()
                msg.info("RumbleLink: ", suburl)
                local appendstr = "append"
                local pos = mp.get_property_number('playlist-pos', 0)
                local plen = mp.get_property_number('playlist-count', 0)
                mp.commandv("loadfile", suburl, appendstr)
    ...
    if not (json.title == nil) then
        ...
        if json['extractor'] then
            ...
            -- mp.set_property("metadata/Album_Artist", extractor)
            -- mp.set_property("file-local-options/extractor", extractor)
            meta.album_artist = extractor
        end
        local uploader = ''
        if json['channel'] and not json['uploader'] then
            json['uploader'] = json['channel']
        end
        if json['uploader'] then
            uploader = '[' .. ( json['uploader'] or "nil" ) .. ']:'
            msg.info("uploader:" .. uploader)
            -- mp.set_property("metadata/Artist", uploader)
            -- mp.set_property("file-local-options/uploader", uploader)
            meta.artist = uploader
        end
        local duration = ''
        if json['duration'] then
            duration = '|' .. ( SecondsToClock(json['duration']) or "nil" ) .. '|'
            -- mp.set_property("file-local-options/duration", duration)
            meta.duration = duration
        end
        local upload_date = ''
        if json['upload_date'] then
            upload_date = '@' .. json['upload_date'] or "nil"
            -- mp.set_property("metadata/Date", upload_date)
            -- mp.set_property("file-local-options/upload-date", upload_date)
            meta.upload_date = upload_date
        end
        local newtitle = (is_playlist and '<p>:' or '') .. uploader .. json['title'] .. duration .. extractor .. upload_date
        newtitle = string.gsub(json.title, "%%", "")
        msg.info("title:" .. newtitle)
        meta.title = newtitle
        -- mp.set_property("media-title", newtitle)
        -- mp.set_property("metadata/Title", newtitle)
        -- mp.set_property("file-local-options/ytdl-title", newtitle)
        -- mp.set_property("file-local-options/force-media-title", newtitle)
    end

    if not (json.webpage_url == nil) then
        -- mp.set_property("file-local-options/webpage-url", json.webpage_url)
        msg.info("webpage-url: " .. json.webpage_url)
        meta.webpage = webpage_url
    end


    -- mp.set_property_native("metadata", meta)
    ...
    elseif (json["_type"] == "playlist")
        ...
        if self_redirecting_url and #json.entries > 1
            ...
            if not (json.title == nil) then
                ...
                if json['extractor'] then
                    extractor = '@' .. json['extractor']
                    -- mp.set_property("metadata/Album_Artist", upload_date)
                    -- mp.set_property("file-local-options/extractor", extractor)
                end
                local uploader = ''
                if json['channel'] and not json['uploader'] then
                ...
                if json['uploader'] then
                    uploader = '[' .. ( json['uploader'] or "nil" ) .. ']:'
                    -- mp.set_property("metadata/Artist", upload_date)
                    -- mp.set_property("file-local-options/uploader", uploader)
                end
                local duration = ''
                if json['duration'] then
                ...
                if json['upload_date'] then
                    upload_date = '@' .. json['upload_date'] or "nil"
                    -- mp.set_property("metadata/Date", upload_date)
                    -- mp.set_property("file-local-options/upload_date", upload_date)
                end
                local newtitle = (is_playlist and '<p>:' or '') .. uploader .. json['title'] .. duration .. extractor .. upload_date
                newtitle = string.gsub(title, "%%", "")
                msg.info("newtitle:" .. newtitle)
                -- mp.set_property("metadata/Title", newtitle)
                -- mp.set_property("media-title", newtitle)
                -- mp.set_property("file-local-options/force-media-title", newtitle)
                msg.info(playlist)
            ...
            if not (json.webpage_url == nil) then
                mp.set_property("file-local-options/webpage_url", json.webpage_url)
                msg.info("webpage_url: " .. json.webpage_url)
            end
            -- if not (json.title == nil) then
