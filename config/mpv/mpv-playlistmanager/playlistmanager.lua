...
function draw_playlist()
  ...
  for index=start,start+settings.showamount-1,1 do
    ...
    local line = parse_filename_by_index(index)
    msg.info(line)
    ass:append(line.."\\N")
...
function pageup()
  for i = 1, settings.showamount, 1 do
    moveup()
  end
end
...
function save_playlist()
  ...
  local cursor = mp.get_property_number('playlist-pos', 0)
  ...
  local savedir = savepath
  local savepath = utils.join_path(savepath, datestring.."_playlist-size_"..length - cursor..".m3u")
  ...
  if not file then
    ...
    file:close()
    local command = {"ln", "-sfv", savepath, utils.join_path(savedir, "latest.m3u")}
    local es, output, result, aborted = exec(command)
    msg.info(output)
    command = {"chown", "-hv", "1000:1000", savepath, utils.join_path(savedir, "latest.m3u")}
    es, output, result, aborted = exec(command)
    msg.info(output)
    command = {"chown", "-v", "1000:1000", savepath, utils.join_path(savedir, "latest.m3u")}
    es, output, result, aborted = exec(command)
    msg.info(output)
  end
end
...
function add_keybinds()
  bind_keys(settings.key_pageup, 'pageup', pageup, "repeatable")
  bind_keys(settings.key_pagedown, 'pagedown', pagedown, "repeatable")
...
function remove_keybinds()
  ...
  if settings.dynamic_binds then
    unbind_keys(settings.key_pageup, 'pageup')
    unbind_keys(settings.key_pagedown, 'pagedown')
...
function resolve_title(target)
  msg.info("resolve_title(" .. target .. ")")
  refresh_globals()
  local i = to_string(target)
  if not settings.resolve_titles then return end
  if target > plen then return end
  local filename = mp.get_property('playlist/'..i..'/filename')
  local title = mp.get_property('playlist/'..i..'/title')
  if title == nil or title == filename then
    local oldtitle = mp.get_property('playlist/'..i..'/media-title')
    if oldtitle ~= nil and oldtitle ~= title then title = oldtitle end
  end
  if    filename
  ...
  then
    ...
    local args = { 'bash', '-c', 'while (( $(readlink /proc/*/exe | grep /usr/bin/python3.9 | wc -l) >= ' ..
      settings.concurrent_par .. ' )) ; do sleep ' .. settings.concurrent_sleep ..  ' ; done ;' ..
      'youtube-dl --no-playlist --flat-playlist -sJ ' .. filename }
          ...
          if res.status == 0 then
            ...
            if not err then
              ...
              if not (json.title == nil) then
                  local extractor = ''
                  if json['extractor'] then extractor = '@' .. json['extractor'] end
                  local uploader = ''
                  if json['uploader'] then uploader = '[' .. ( json['uploader'] or "nil" ) .. ']:' end
                  local duration = ''
                  if json['duration'] then duration = '|' .. ( SecondsToClock(json['duration']) or "nil" ) .. '|' end
                  local upload_date = ''
                  if json['upload_date'] then upload_date = '@' .. json['upload_date'] or "nil" end
                  newtitle = (is_playlist and '<p>:' or '') .. uploader .. json['title'] .. duration .. extractor .. upload_date
              end
              if newtitle ~= nil then title = newtitle end
              msg.verbose(filename .. " resolved to '" .. title .. "'")
              url_table[filename] = title
              mp.set_property('playlist/'..i..'/media-title', title)
              refresh_globals()
              if playlist_visible then showplaylist() end
              return
            else
    ...
    mp.add_timeout(360, function()
      mp.abort_async_command(req)
      requested_urls[filename] = nil
    end)
  end
end
...
mp.register_script_message("resolvetitles", resolve_titles)
...
mp.add_key_binding("F3", "resolvetitles", resolve_titles)
